...
class Operation:
    ...
    def  TestOneBoundAllPagesCompareActList(self, bound,behavList,isSame=True):
        ...
        for behav in behavList:
            ...
            if (not self.CheckIsSame(fir_cli_CenterImg,preActImg,self.simiLari)): #r如果两个页面不一样
                ...
                print('oneSub',oneSub)
                print('oneRec',oneRec)
                print('oneProNum',oneProNum)
                print('oneRom',oneRom)

                if (abs(oneSub) > 25000 or abs(oneRec) > 29000 or abs(oneProNum) > 3 or abs(oneRom) > 3072):
                    if len(imgList)>0:
                        misOpe.append((cliX, cliY, behav))
                        self.timer.oraOver()
                        return misOpe
                ...
                for sta in statusList:
                    AvgRec = AvgRec + sta[0]-theStatus[0]
                    AvgSub = AvgSub + sta[0]-theStatus[0]
                    AvgProNum = AvgProNum + sta[0]-theStatus[0]
                    AvgRom = AvgRom + sta[0]-theStatus[0]

                if(len(statusList)!=0):
                    AvgRec = AvgRec/len(statusList)
                    AvgSub = AvgSub/len(statusList)
                    AvgProNum = AvgProNum/len(statusList)
                    AvgRom = AvgRom/len(statusList)

                if (abs(AvgRec) > 19000  or abs(AvgSub) > 15000 or abs(AvgProNum) > 2 or abs(AvgRom) > 2048):
            ...
            if len(imgList) != 0:
                for cenImg in imgList:
                    if (not self.CheckIsSame(fir_cli_CenterImg, preActImg, self.simiLari)):
                        ...
                        misOpe.append([cliX, cliY, behav])
                        print((cliX, cliY, behav))
                        self.AddActiToList([cliX, cliY, behav])
                        if isSame:
                            if (not self.CheckIsSame(fir_cli_CenterImg, cenImg, self.simiLari)):
                                misOpe.append((cliX, cliY, behav))
                                break
                        else:
                            if (self.CheckIsSame(fir_cli_CenterImg, cenImg, self.simiLari)):
